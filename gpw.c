/* GPW - Generate pronounceable passwords
   ...
   Original author unknown - DrJ 3/15/13
   Copyright 2013, DrJ,  Artistic License 2.0, http://opensource.org/licenses/Artistic-2.0
   */
...
int main (int argc, char ** argv) {
	...
	password_length = 8;		/* Default value for password length. Originally was 8 */
	...
	srand (uint(systime.tv_usec)); /* Seed for rand() */
	...
	for (pwnum=0; pwnum < n_passwords; pwnum++) {
		...
		/* Insert random number or symbol */
		nchar = rand_lim(password_length);
		password[nchar] = '!' + rand_lim(31);
		/* Change case of random character */
		int nchar_prev = nchar;
		do {
			nchar = rand_lim(password_length);
		} while (nchar == nchar_prev);
		password[nchar] = password[nchar] - 32;
		printf ("%s\n", password);
	}								/* for pwnum */
	exit (0);
}
